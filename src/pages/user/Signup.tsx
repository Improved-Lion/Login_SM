  ...
  async function addUser(formData: UserForm) {
    try {
      ...
      if (resData.ok) {
        ...
        //& API 서버의 에러 메시지 처리 - 서버에서 실패한 에러 값을 반환하는 부분
      ...
      <div className='p-8 border border-gray-200 rounded-lg w-full max-w-md dark:bg-gray-600 dark:border-0'>
        ...
        <form action='/' onSubmit={handleSubmit(addUser)}>
          ...
          <div className='mb-4'>
            ...
            <input type='file' id='attach' accept='image/*' placeholder='이미지를 선택하세요' className='w-full px-3 py-2 border rounded-lg dark:bg-gray-700' {...register('attach')} />
          </div>
          <div className='mt-10 flex justify-center items-center'>
            <Submit>회원가입</Submit>
            <a href='/' className='bg-gray-900 py-1 px-4 text-base text-white font-semibold ml-2 hover:bg-amber-400 rounded'>
              취소
            </a>
          </div>
        </form>
