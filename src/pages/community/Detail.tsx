      ...
      <section className='mb-8 p-4'>
        <form action={`/${type}`}>
          ...
          <div className='flex justify-end my-4'>
            <Link to={`/${type}`} className='bg-orange-500 py-1 px-4 text-base text-white font-semibold ml-2 hover:bg-amber-400 rounded'>
              목록
            </Link>
            {user?._id === data?.user._id && (
              <>
                <Link to={`/${type}/${_id}/edit`} className='bg-gray-900 py-1 px-4 text-base text-white font-semibold ml-2 hover:bg-amber-400 rounded'>
                  수정
                </Link>
